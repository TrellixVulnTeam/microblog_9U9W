...
class TestCase(unittest.TestCase):
    ...
    def test_follow_posts(self):
        ...
        db.session.add(u4)
        # make 4 posts
        p1 = Post(body='post from john',  author=u1, timestamp=datetime.utcnow()+timedelta(seconds=1))
        p2 = Post(body='post from susan', author=u2, timestamp=datetime.utcnow()+timedelta(seconds=2))
        p3 = Post(body='post from mary',  author=u3, timestamp=datetime.utcnow()+timedelta(seconds=3))
        p4 = Post(body='post from david', author=u4, timestamp=datetime.utcnow()+timedelta(seconds=4))
        db.session.add(p1)
        db.session.add(p2)
        db.session.add(p3)
        db.session.add(p4)
